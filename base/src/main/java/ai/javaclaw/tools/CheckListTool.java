...
public class CheckListTool {
    ...
    private void validateCheckList(CheckList checkList) {
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if (checkListItem.content() == null || checkListItem.content().isBlank()) {
                throw new IllegalArgumentException("Checklist item at index " + i
                        + " has empty or blank content. All taskList must have meaningful content.");
            }
        ...
        long inProgressCount = items.stream()
                .filter(item -> item.status() == CheckList.Status.in_progress)
                .count();

        if (inProgressCount > 1) {
            throw new IllegalArgumentException("Only ONE checklist item can be in_progress at a time. Found "
                    + inProgressCount + " in_progress checklist items. "
                    + "Please mark the current checklist item as completed before starting a new one.");
        }
    }
    ...
    public static class Builder {
        ...
        private CheckListEventHandler checkListEventHandler =
                checkList -> logger.debug("Updated checklist: {}", checkList);
