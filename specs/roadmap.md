...
- 15.5.3 Shared conversations ✅ — V35 миграция conversation_shares (conversation_id+shared_with unique, permission READ/WRITE, FK CASCADE), ConversationShare entity/repository, ConversationSharingService (share/unshare/listShares/hasAccess/hasWriteAccess/getSharedConversationIds), ConversationQueryService.listConversationsWithShared (owned + shared via subquery), ConversationController расширен (POST /{id}/share, DELETE /{id}/share/{username}, GET /{id}/shares, PUT /{id}/share), messages endpoint использует hasAccess для shared read, 21 новый тест
- 15.5.5 Admin conversation access ✅ — ConversationSharingService расширен PermissionService dependency + admin-aware overloads (hasAccess/hasWriteAccess/listShares/unshare с username param), isAdmin() проверяет CONVERSATION_ACCESS_ALL permission, ConversationController использует admin bypass (list all conversations, read any messages, delete any conversation, manage any shares), ConversationIsolationIntegrationTest обновлён для admin-sees-all, 9 новых unit-тестов в AdminConversationAccess nested class + 1 новый controller test
- 15.6.1 Agent assignment per role ✅ — V36 миграция role_agent_config (role/model_id/thinking_enabled/thinking_budget/fallback_models/max_context_tokens), RoleAgentConfig entity/repository, RoleAgentConfigService (resolveForRole с hierarchy fallback USER→POWER_USER→ADMIN, кэш, CRUD), UserResolver.resolveUserRole(), ChatService расширен 4-arg stream/call с modelOverride, SseStreamingService 5-arg stream с modelOverride, ChatRestController резолвит роль→модель через RoleAgentConfigService, RoleController расширен GET/PUT/DELETE /{name}/agent-config + GET /agent-configs, 26 новых тестов (7 entity + 19 service), все 1147 тестов зелёные
- 15.6.4 Model access per role ✅ — V38 миграция role_model_allowlist (role+model_id UNIQUE), RoleModelAllowlist entity/repository, RoleModelAllowlistService (getAllowedModels/isModelAllowed/validateModelAccess с кэшем, setAllowedModels/addAllowedModel/removeAllowedModel, ModelAccessDeniedException), ChatRestController валидирует model access перед streaming (403 при запрете), RoleController расширен GET/PUT/POST/DELETE /{name}/allowed-models endpoints, 27 новых unit-тестов + 1 новый controller test, все 1187 тестов зелёные
